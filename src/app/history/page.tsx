  ...
  useEffect(() => {
    async function loadHistory() {
      try {
        ...
        // Transform db format to match UI expected format
        const formatted = data.map((item: any) => ({
          id: item.id,
          date: format(new Date(item.createdAt), "MMM d, yyyy"),
          industry: item.industry || "General",
          useCase: item.businessGoals || "General AI Initiative",
          roi: item.roiForecast || 0,
      ...
      <main style={{ flex: 1, overflow: "hidden", display: "flex", flexDirection: "column" }}>
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8 space-y-5">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            <MetricPill icon={Briefcase}  label="Total Analyses"    value={loading ? "-" : `${allAnalyses.length}`}  accent="#1a3a5c" />
            <MetricPill icon={TrendingUp} label="Avg Projected ROI" value={loading ? "-" : isNaN(parseFloat(avgRoi)) ? "0%" : `${avgRoi}%`} accent="#10B981" />
            <MetricPill icon={Activity}   label="Avg Readiness"     value={loading ? "-" : isNaN(parseFloat(avgReadiness)) ? "0" : `${avgReadiness}`} subtext="/100" accent="#c8a96e" />
            <MetricPill icon={PieChart}   label="Top Industry"      value={loading ? "-" : (industries.length > 1 ? industries[1] : "N/A")} accent="#f59e0b" />
          </div>
          ...
          {loading ? (
            <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4 mb-10">
              {[1, 2, 3, 4, 5, 6].map((i) => (
                <div key={i} className="bg-white border border-gray-200 p-5 h-[220px] animate-pulse">
                  <div className="h-4 bg-gray-200 w-1/4 mb-4 rounded" />
                  <div className="h-6 bg-gray-200 w-3/4 mb-4 rounded" />
                  <div className="h-4 bg-gray-200 w-1/3 mb-6 rounded" />
