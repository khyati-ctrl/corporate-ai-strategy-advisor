...
export default function TopBar({ title, subtitle, actions }: TopBarProps) {
  ...
  const userName = session?.user?.name || "Jane Doe";
  const userInitials = userName.split(" ").map(n => n[0]).join("").substring(0, 2).toUpperCase();

  return (
    <div className="relative flex flex-col md:flex-row items-start md:items-center justify-between gap-5 p-4 pl-20 md:py-6 md:pr-9 md:pl-20 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800">
          ...
          <Link href="/profile" style={{ textDecoration: "none" }}>
            ...
            >
              ...
              <div className="hidden sm:flex flex-col justify-center">
                <span style={{ fontSize: "0.875rem", fontWeight: 700, color: "#111827", lineHeight: 1.2 }}>{userName}</span>
